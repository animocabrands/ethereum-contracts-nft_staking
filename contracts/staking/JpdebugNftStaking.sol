...
abstract contract JpdebugNftStaking is Ownable, Pausable, ERC1155TokenReceiver {
    ...
    // a struct container used to track a staker's snapshot staking info
    struct StakerSnapshot {
        uint64 depositCycle; // cycle from which a staker may claim dividend rewards for the snapshot NFTs
        uint64 stakedWeight; // current snapshot staked weight of NFTs staked by the staker
    }
    ...
    mapping(address => StakerSnapshot) public stakerSnapshots; // staker address => staker snapshot
    ...
    function _getOrCreateLatestCycleSnapshot(uint256 offsetIntoFuture) internal returns(DividendsSnapshot storage, uint256) {
        ...
        return _addNewSnapshot(currentCycle, currentCycle, readSnapshot.stakedWeight, readSnapshot.tokensToClaim);
    }
    ...
    */
    ...
    /**
     * Claims the dividends for the specified number of payout periods.
     * @param numPayoutPeriodsToClaim The maximum number of dividend payout periods to claim for.
     */
    function jpdebugClaimDividends(uint256 numPayoutPeriodsToClaim) external isEnabled {
        // claiming for no payout periods
        if (numPayoutPeriodsToClaim == 0) {
            return;
        }

        StakerState memory stakerState = stakeStates[msg.sender];
        ...
        // no snapshots to claim from
        if (dividendsSnapshots.length == 0) {
            return;
        }

        uint256 numCyclesToClaim = getCurrentCycle().sub(stakerState.depositCycle);

        // insufficient cycles to be able to claim a single payout period
        if (numCyclesToClaim < payoutPeriodLength) {
            return;
        }

        (DividendsSnapshot memory snapshot, uint256 snapshotIndex) =
            _findDividendsSnapshot(stakerState.depositCycle);

        require(
            snapshot.cycleRangeEnd <= stakerState.depositCycle,
            "NftStaking: Dividends snapshot does not exist for the staker deposit cycle");

        if (snapshot.cycleRangeStart < stakerState.depositCycle) {
            (snapshot, snapshotIndex) = _getOrCreateLatestCycleSnapshot(0);
        }

        uint256 startingSnapshotIndex = snapshotIndex;

        numPayoutPeriodsToClaim = Math.min(
            numPayoutPeriodsToClaim,
            numCyclesToClaim.div(payoutPeriodLength));

        numCyclesToClaim = numPayoutPeriodsToClaim.mul(payoutPeriodLength);

        uint256 dividendsAmount = 0;

        for (uint256 cycleCount = 0; cycleCount < numCyclesToClaim; ) {
            uint256 numSnapshotCycles = snapshot.cycleRangeEnd - snapshot.cycleRangeStart + 1;
            uint256 numSnapshotCyclesToClaim = Math.min(
                numSnapshotCycles,
                numCyclesToClaim - cycleCount);

            /// JPDEBUG: FIX THIS SHIT
            uint256 temp = SafeMath.mul(stakerState.stakedWeight, DIVS_PRECISION);
            temp = temp.div(snapshot.stakedWeight);
            temp = temp.mul(snapshot.tokensToClaim);
            temp = temp.mul(numSnapshotCyclesToClaim);
            temp = temp.div(numSnapshotCycles);
            temp = temp.div(DIVS_PRECISION);

            dividendsAmount = dividendsAmount.add(temp);

            cycleCount = cycleCount.add(numSnapshotCyclesToClaim);
        }

        stakeStates[msg.sender].depositCycle = SafeMath.add(stakerState.depositCycle, numCyclesToClaim).toUint64();

        if (dividendsAmount > 0) {
            // must never underflow
            require(IERC20(dividendToken).balanceOf(address(this)) >= dividendsAmount, "NftStaking: Insufficient tokens in the rewards pool");
            require(IERC20(dividendToken).transfer(msg.sender, dividendsAmount), "NftStaking: Unknown failure when attempting to transfer claimed dividend rewards");

            emit ClaimedDivs(msg.sender, startingSnapshotIndex, snapshotIndex, dividendsAmount);
        }
    }
    ...
    */
    ...
    function _depositNft(uint256 tokenId, address tokenOwner) internal isEnabled whenNotPaused {
        ...
        uint64 nftWeight = valueStakeWeights[_valueFromTokenId(tokenId)].toUint64();
        ...
        uint64 currentCycle = getCurrentCycle().toUint64();

        tokenInfo.depositCycle = currentCycle.toUint64();
        tokensInfo[tokenId] = tokenInfo;

        StakerSnapshot[] storage stakerSnapshots_ = stakerSnapshots[tokenOwner];
        uint256 numStakerSnapshots = stakerSnapshots_.length;

        // no tokens currently staked by the user
        if (numStakerSnapshots == 0) {
            StakerSnapshot storage newStakerSnapshot = stakerSnapshots_.push();
            newStakerSnapshot.depositCycle = currentCycle;
            newStakerSnapshot.stakedWeight = nftWeight;
        } else {
            StakerSnapshot storage stakerSnapshot = stakerSnapshots_[numStakerSnapshots - 1];

            // last snapshot is for the current cycle
            if (stakerSnapshot.depositCycle == currentCycle) {
                stakerSnapshot.stakedWeight = SafeMath.add(stakerSnapshot, nftWeight).toUint64();
            } else {
                StakerSnapshot storage newStakerSnapshot = stakerSnapshots_.push();
                newStakerSnapshot.depositCycle = currentCycle;
                newStakerSnapshot.stakedWeight = SafeMath.add(stakerSnapshot.stakedWeight, nftWeight).toUint64();
            }
        }

        emit Deposit(tokenOwner, tokenId, currentCycle);
    }
